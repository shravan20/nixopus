...
class TestDockerCommandBuilder:
    ...
    def test_build_restart_command_with_all_parameters(self):
        ...
        assert cmd == ["docker", "compose", "-f", "/path/to/docker-compose.yml", "restart", "--env-file", "/path/to/.env", "api"]
...
class TestDockerService:
    ...
    def test_restart_services_success(self, mock_run):
        ...
        self.logger.info.assert_called_once_with("Restarting services: web")
        self.logger.success.assert_called_once_with("Services restarted successfully: web")
...
class TestRestart:
    ...
    def test_restart_success(self):
        ...
        with patch("app.commands.service.restart.RestartService.execute", return_value=RestartResult(
            name=config.name,
